...
const Statistics = ({
  ...
  // Function to fetch all reviews based on filters (Company Admin Scope)
  const fetchCompanyReviewsForStatistics = useCallback(async () => {
    setIsLoading(true);
    setError('');
    try {
      ...
      if (response.ok) {
        ...
        console.log("Statistics (Company Admin): Reviews fetched for statistics:", data);
      } else {
        setError(data.message || 'Failed to fetch reviews for statistics.');
        setReviews([]);
        console.error("Statistics (Company Admin): Failed to fetch reviews for statistics:", data.message);
      }
    } catch (err) {
      console.error('Statistics (Company Admin): Error fetching reviews for statistics:', err);
  ...
  useEffect(() => {
    if (userData?.token && userData?.companyId) {
      fetchCompanyReviewsForStatistics();
  ...
  }, [filterBranchId, filterClientId, filterStartDate, filterEndDate, userData, fetchCompanyReviewsForStatistics, fetchClientsForCompanyAdminFilters]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div>
          ...
          <select
            ...
            disabled={filteredClients.length === 0 && filterBranchId !== ''}
          >
