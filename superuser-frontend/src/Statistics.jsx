...
const PIE_COLORS = ['#4CAF50', '#FFC107', '#F44326']; // Green, Amber, Red (corrected a typo in F4436 to F44326)
...
const Statistics = ({
  ...
  // States for the filter dropdowns (these are local to Statistics)
  ...
  // Function to fetch clients and branches specifically for the statistics filters
    ...
    try {
      ...
      if (companyId) {
        ...
        if (branchResponse.ok) {
          setBranchesForStatsFilter(branchData);
        } else {
          setError(branchData.message || 'Failed to fetch branches for filter.');
          setBranchesForStatsFilter([]);
        }
      } else {
        setBranchesForStatsFilter([]); // Clear branches if no company selected
      }
      ...
      if (branchId) {
        ...
        clientUrl = `${API_BASE_URL}/superuser/clients`; // Fetch all clients for the filter dropdown
      }
      ...
      if (clientResponse.ok) {
        setClientsForStatsFilter(clientData);
      } else {
        setError(clientData.message || 'Failed to fetch clients for filter.');
        setClientsForStatsFilter([]);
      }
    } catch (err) {
      console.error('Statistics: Error fetching clients/branches for statistics filters:', err);
      setError('Network error fetching filter data.');
    }
  }, [API_BASE_URL, getAuthHeaders, setError]);

  // Function to fetch all reviews based on filters (similar to ViewReviews but for stats)
  const fetchReviewsForStatistics = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Statistics: Reviews fetched for statistics:", data);
      } else {
        setError(data.message || 'Failed to fetch reviews for statistics.');
        setReviews([]);
        console.error("Statistics: Failed to fetch reviews for statistics:", data.message);
      }
    } catch (err) {
      console.error('Statistics: Error fetching reviews for statistics:', err);
  ...
  return (
    <div className="p-6 bg-blue-50 rounded-lg shadow-inner">
      {/* Recharts CDN removed as it should be installed via npm/yarn */}

      <h4 className="text-xl font-semibold text-blue-800 mb-4">Review Statistics</h4>

      {/* Filter Section (Copied from ViewReviews for consistency) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div>
          <label htmlFor="filterCompanyStats" className="block text-sm font-medium text-gray-700">Filter by Company:</label>
          <select
            id="filterCompanyStats"
        ...
        <div>
          <label htmlFor="filterBranchStats" className="block text-sm font-medium text-gray-700">Filter by Branch:</label>
          <select
            id="filterBranchStats"
            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          ...
          >
            ...
            {branchesForStatsFilter.map(branch => (
        ...
        <div>
          ...
          >
            ...
            {clientsForStatsFilter.map(client => (
      ...
      {!isLoading && reviews.length === 0 && !error && (
        <p className="text-gray-600 text-center py-4">No reviews found to generate statistics. Adjust your filters or add some reviews.</p>
      )}
